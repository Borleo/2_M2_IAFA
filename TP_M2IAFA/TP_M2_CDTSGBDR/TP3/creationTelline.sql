...
AS SELECT EMPT.*, datemb,sal,comm, supr
FROM EMPT, EMPB@brehat EMPB
WHERE EMPT.noemp = EMPB.noemp;

CREATE OR REPLACE VIEW ProjetT
AS SELECT * FROM ProjetT
UNION SELECT * FROM ProjetB@brehat;

CREATE OR REPLACE VIEW Dept
AS SELECT * FROM Dept@brehat;

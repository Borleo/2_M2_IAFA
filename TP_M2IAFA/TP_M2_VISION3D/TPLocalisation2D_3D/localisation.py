...
def transform_and_draw_model(edges_Ro, intrinsic, extrinsic, fig_axis): # Ro repère de l'objet
    # ********************************************************************* #
    # A COMPLETER.                                                          #
    ...
    u1,v1 = perspective_projection(intrinsic, P1_Rc)
    u2,v2 = perspective_projection(intrinsic, P2_Rc)
...
def perspective_projection(intrinsic, P_c): # repère dans les coordonnées caméra P_c
    # ***************************************************** #
    # A COMPLETER.                                          #
...
def calculate_normal_vector(p1_Ri, p2_Ri, intrinsic):
    ...
    # ********************************************************* #
    # Pi = K.Pc => Pcamra = K^-1.Pimg deux points + [0,0,0] origine donne un plan
    # N. = l1∧l2 / ∣∣l1∧l2||
    # np.dot(a, b) # produit vectoriel
    # np.linalg.norm # Calcule une norme de vecteur ou de matrice
    # np.cross # Produit vectoriel, principalement pour des vecteurs 3D
    
    p1_Rc = np.dot(np.linalg.inv(intrinsic), p1_Ri)
    p2_Rc = np.dot(np.linalg.inv(intrinsic), p2_Ri)
    l1 = p1_Rc - np.array([0,0,0])  
    l2 = p1_Rc - p2_Rc 
    norme_prod_vect = np.linalg.norm(l1,l2)
    normal_vector = np.cross(l1,l2) / np.linalg.norm(l1,l2)
...
def calculate_error(nb_segments, normal_vectors, segments_Rc):
    ...
    p2 = segments_Rc[:,0:6].T
    for p in range(nb_segments):
        # Partie a completer avec le calcul de l'erreur cumulee
        err_A = np.power(np.dot(p1[p],normal_vectors[:,p]),2) 
        err_B = np.power(np.dot(p2[p],normal_vectors[:,p]),2)
...
def partial_derivatives(normal_vector, P_c):
    ...
    partial_derivative[3] = np.dot(normal_vector, np.array([1, 0, 0]))
    partial_derivative[4] = np.dot(normal_vector, np.array([0, 1, 0]))
    partial_derivative[5] = np.dot(normal_vector, np.array([0, 0, 1]))
    # ********************************************************************

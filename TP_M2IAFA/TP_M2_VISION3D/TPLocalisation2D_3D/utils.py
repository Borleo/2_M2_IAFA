...
path = "/Users/asriel/TP_IAFA/M2_IAFA/TP_M2IAFA/TP_M2_VISION3D/TPLocalisation2D_3D/data/"
...
def select_segments():
    ...
    def on_pick(event):
        ...
        if event.artist in lines:
            ...
            ax1.text((X1_Ro[ind] + X2_Ro[ind]) / 2, (Y1_Ro[ind] + Y2_Ro[ind]) / 2, (Z1_Ro[ind] + Z2_Ro[ind]) / 2, str(iPick), color=line_color[iPick])
            iPick += 1
            fig1.canvas.draw()
    ...
    filepath_pikachu= os.path.join(path,"pikachu.xyz") 
    filepath_edges= os.path.join(path,"pikachu.edges") 

    model_points_3DRo = np.loadtxt(filepath_pikachu, dtype=float)
    model_edges = np.loadtxt(filepath_edges, dtype=int)
    ...
    filepath_pikachu_dino= os.path.join(path,"pikachu_dino.xyz") 
    model_points_3DRo_final = np.loadtxt(filepath_pikachu_dino, dtype=float)

    filepath_pikachu_dino_edge= os.path.join(path,"pikachu_dino.edges") 
    model_edges_final = np.loadtxt(filepath_pikachu_dino_edge, dtype=int)
    ...
    calibration_parameters= os.path.join(path,"calibration_parameters.txt") 
    Rc_to_Rc2_parameters= os.path.join(path,"Rc_to_Rc2_matrix.txt")    
    print(calibration_parameters)

    intrinsic_matrix, Rc_to_Rc2_matrix, Rc2_to_Rc_matrix = \
        get_calib_params(calibration_parameters, Rc_to_Rc2_parameters)
    ...
    image_1= os.path.join(path,"image_1.JPG")
    image_2= os.path.join(path,"image_2.JPG")

    image = mpimg.imread(image_1)
    image_2 = mpimg.imread(image_2)

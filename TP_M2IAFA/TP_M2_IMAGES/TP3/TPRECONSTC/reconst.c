...
Matrix Reconstruction(Matrix _MatPg, Matrix _MatPd, Matrix _Matp2g, Matrix _Matp2d)
{
  ...
  _mp3g = MatAlloc(Double,n_p2g,n_pg); 
  double **mp3g = MatGetDouble(_mp3g);
...
------------------------------------------------------------------*/
  for (int i = 0 ; i < n_p2g ; i++ )
    { 
    _A  = MatAlloc(Double,4,4);
    if (_A==NULL)
    {
      Erreur("Mult : problème d'allocation mémoire");
      return NULL;
    ...
    A[0][0] = MatPg[0][0]  - (Matp2g[i][0] * MatPg[2][0]);
    A[0][1] = MatPg[0][1]  - (Matp2g[i][0] * MatPg[2][1]);
    A[0][2] = MatPg[0][2]  - (Matp2g[i][0] * MatPg[2][2]);
    A[0][3] =-((Matp2g[i][0] *  MatPg[2][3]) - MatPg[0][3]);

    A[1][0] = MatPg[1][0]  - (Matp2g[i][1] * MatPg[2][0]);
    A[1][1] = MatPg[1][1]  - (Matp2g[i][1] * MatPg[2][1]);
    A[1][2] = MatPg[1][2]  - (Matp2g[i][1] * MatPg[2][2]);
    A[1][3] =-((Matp2g[i][1] *  MatPg[2][3]) - MatPg[1][3]);

    A[2][0] = MatPd[0][0]  - (Matp2d[i][0] * MatPd[2][0]);
    A[2][1] = MatPd[0][1]  - (Matp2d[i][0] * MatPd[2][1]);
    A[2][2] = MatPd[0][2]  - (Matp2d[i][0] * MatPd[2][2]);
    A[2][3] =-((Matp2d[i][0] *  MatPd[2][3]) - MatPd[0][3]); 

    A[3][0] = MatPd[1][0] - (Matp2d[i][1] * MatPd[2][0]);
    A[3][1] = MatPd[1][1] - (Matp2d[i][1] * MatPd[2][1]);
    A[3][2] = MatPd[1][2] - (Matp2d[i][1] * MatPd[2][2]);
    A[3][3] =-((Matp2d[i][1] * MatPd[2][3])  - MatPd[1][3]);
  ...
  */
    ...
    int m_pAT    = MatNbCol(AT);
    MatWriteAsc(AT, "");
 
    printf("\n\n ligne 283 - AT*A nombre ligne %d, colonne : %d\n",n_pAT,m_pAT);

    _ATA         = MatAlloc(Double,n_pAT,m_pA);
    _ATA         = Mult(AT,_A);
    ...
    */
    printf("\n\nLigne 296 - Identification du vecteur propre associé à la plus petite valeur propre de ATA");
    ...
    */
    ...
    MatWriteAsc(pVal, "");
    printf("\nLigne 318 - l'Indice %d de la plus petite valeur % .16e des valeurs propres\n",n_ppepc, p_Val[n_ppepc]);
    printf("\n\n Ligne 319 - pVec contient les vecteurs propres du système ATA\n");
    MatWriteAsc(pVec, ""); 
    ...
    _ppVec = MatAlloc(Double,n_pVec,1); 
    double **p_ppVec = MatGetDouble(_ppVec);
    double **p_pVec = MatGetDouble(pVec);
 
    double norm = 0.0; 
 
    for ( int i = 0 ; i < n_pVec ; i++ )
      {
      *p_ppVec[i] = p_pVec[i][n_ppepc];
      norm = p_pVec[i][n_ppepc]; 
      }

    printf("\nLigne 341 - % .16e est la valeur de Cette normalisation du vecteur propre unitaire\n",norm); 
    MatWriteAsc(_ppVec, ""); 
    ...
    /*Les coordonnées 3D sont données par le vecteur propre correspondant à la plus petite valeur propre, normalisé */
    _pestmx = MatAlloc(Double,3,1); 
    double **p_pestmx = MatGetDouble(_pestmx);

    for ( int i = 0 ; i < 3 ; i++ )
        *p_pestmx[i] = p_ppVec[i][0]/norm;

    printf("\nLigne 354 - L’estimée Pb de la matrice de projection perspective après normalisation vaut\n"); 
    MatWriteAsc(_pestmx, "");   

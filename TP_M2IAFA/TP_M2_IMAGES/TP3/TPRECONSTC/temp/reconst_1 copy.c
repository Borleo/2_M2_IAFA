...
/*
  ...
  */
  ...
  Matrix _ppVec = MatAlloc(Double,n_pVec,1); 
  ...
  for ( int i = 0 ; i < n_pVec ; i++ )
    {
      *p_ppVec[i] = p_pVec[i][n_ppepc];
      norm += p_pVec[i][n_ppepc] * p_pVec[i][n_ppepc]; 
    }
  ...
  Matrix _pestmx = MatAlloc(Double,3,1); 
  ...
  for ( int i=0 , j=0 ; i < n_p2d ; i+=1, j+=4 )
   { 
    A[j][0] = MatPg[0][0]  - (Matp2g[i][0] * MatPg[2][0]) * X;
    A[j][1] = MatPg[0][1]  - (Matp2g[i][0] * MatPg[2][1]) * Y;
    A[j][2] = MatPg[0][2]  - (Matp2g[i][0] * MatPg[2][2]) * Z;

    A[j+1][0] = MatPg[1][0]  - (Matp2g[i][1] * MatPg[2][0]) * X;
    A[j+1][1] = MatPg[1][1]  - (Matp2g[i][1] * MatPg[2][1]) * Y;
    A[j+1][2] = MatPg[1][2]  - (Matp2g[i][1] * MatPg[2][2]) * Z;
  
    A[j+2][0] = MatPd[0][0]  - (Matp2d[i][0] * MatPd[2][0]) * X;
    A[j+2][1] = MatPd[0][1]  - (Matp2d[i][0] * MatPd[2][1]) * Y;
    A[j+2][2] = MatPd[0][2]  - (Matp2d[i][0] * MatPd[2][2]) * Z;

    A[j+3][0] = MatPd[1][0] - (Matp2d[i][1] * MatPd[2][0]) * X;
    A[j+3][1] = MatPd[1][1] - (Matp2d[i][1] * MatPd[2][1]) * Y;
    A[j+3][2] = MatPd[1][2] - (Matp2d[i][1] * MatPd[2][2]) * Z;
    }
...
Matrix GetMatrix(Matrix _MatPg, Matrix _MatPd, Matrix _Matp2g,
                 Matrix _Matp2d, double ** A, double **B)
{
  int n_pg       = MatNbRow(_MatPg);
  int m_pg       = MatNbCol(_MatPg);
  double **MatPg = MatGetDouble(_MatPg);
  ...
  double **pestmx    = MatGetDouble(_pestmx);
  double X = *pestmx[0];
  double Y = *pestmx[1];
  double Z = *pestmx[2];

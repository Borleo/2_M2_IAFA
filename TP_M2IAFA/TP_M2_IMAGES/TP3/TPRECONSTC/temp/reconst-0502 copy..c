...
------------------------------------------------------------------*/
  ...
  Matrix _A      = MatAlloc(Double,4*n_p2d,4);
  if (_A==NULL)
  {
    Erreur("Mult : problème d'allocation mémoire");
    return NULL;
...
/*
  ...
  */
  ...
  Matrix _mp3g = MatAlloc(Double,n_p2g,3); 
  double **mp3g = MatGetDouble(_mp3g);

  Matrix _mp3d = MatAlloc(Double,n_p2d,3); 
  double **mp3d = MatGetDouble(_mp3d);
  ...
  printf("\n\n ligne 383 - matp3d_verif ");
  Matrix _matp3d_verif=MatReadAsc("p3d-verif.mx");
  MatWriteAsc(_matp3d_verif, "");
